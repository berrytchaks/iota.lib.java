 ...
 * <p>
 * {@code IotaApiProxy api = IotaApiProxy.Builder} {@code .protocol("http")}
 * {@code .nodeAddress("localhost")} {@code .port(12345)} {@code .build();}
 *
...
public class IotaAPI extends IotaAPICore {
	...
	/**
	 * Generates a new address from a seed and returns the remainderAddress. This is
	 * either done deterministically, or by providing the index of the new
	 * remainderAddress.
	 *
	 * @param seed
	 *            Tryte-encoded seed. It should be noted that this seed is not
	 *            transferred.
	 * @param security
	 *            Security level to be used for the private key / address. Can be 1,
	 *            2 or 3.
	 * @param index
	 *            Key index to start search from. If the index is provided, the
	 ...
	 * @param returnAll
	 *            If <code>true</code>, it returns all addresses which were
	 ...
	 *             is thrown when the specified input is not valid.
	 */
	public GetNewAddressResponse getNewAddress(final String seed, int security, final int index, final boolean checksum,
			final int total, final boolean returnAll) throws ArgumentException {
		StopWatch stopWatch = new StopWatch();

		List<String> allAddresses = new ArrayList<>();
		// If total number of addresses to generate is supplied, simply generate
		// and return the list of all addresses
		int newAddressIndex = index;
		if (total != 0) {
		...
		if (!returnAll) {
			...
			// allAddresses = allAddresses.subList(allAddresses.size() - 2,
			// allAddresses.size() - 1);
	 ...
	 *            If <code>true</code>, it gets the inclusion states of the
	 *            transfers.
	 * @return Bundle of transfers.
	 * @throws ArgumentException
	 *             is thrown when the specified input is not valid.
	 */
	public GetTransferResponse getTransfers(String seed, int security, Integer start, Integer end,
			Boolean inclusionStates) throws ArgumentException {
		...
		if (gnr != null && gnr.getAddresses() != null) {
			Bundle[] bundles = bundlesFromAddresses(gnr.getAddresses().toArray(new String[gnr.getAddresses().size()]),
					inclusionStates);
	...
	public Bundle[] bundlesFromAddresses(String[] addresses, final Boolean inclusionStates) throws ArgumentException {
		...
		List<Transaction> bundleObjects = findTransactionObjectsByBundle(
				nonTailBundleHashes.toArray(new String[nonTailBundleHashes.size()]));
		...
		Parallel.For(Arrays.asList(tailTxArray), new Parallel.Operation<String>() {
			public void perform(String param) {
				...
				try {
					...
					if (gbr.getTransactions() != null) {
						if (inclusionStates) {
							...
							if (finalInclusionStates != null) {
								thisInclusion = finalInclusionStates.getStates()[Arrays.asList(tailTxArray)
										.indexOf(param)];
							}
					...
					// If error returned from getBundle, simply ignore it because the bundle was
		...
		for (int i = 0; i < finalBundles.size(); i++) {
			returnValue[i] = new Bundle(finalBundles.get(i).getTransactions(),
					finalBundles.get(i).getTransactions().size());
		}
		return returnValue;
	}
	...
	/**
	 * Facade method: Gets transactions to approve, attaches to Tangle, broadcasts
	 * and stores.
	 *
	 * @param trytes
	 *            The trytes.
	 ...
	 * @throws ArgumentException
	 *             is thrown when invalid trytes is provided.
	 */
	public List<Transaction> sendTrytes(final String[] trytes, final int depth, final int minWeightMagnitude)
		...
		final GetAttachToTangleResponse res = attachToTangle(txs.getTrunkTransaction(), txs.getBranchTransaction(),
	 ...
	 * Returns the transactionObject of a transaction hash. The input can be a list
	 * of valid addresses.
	 *
	 * @param addresses
	 *            The addresses.
	...
	public List<Transaction> findTransactionObjectsByAddresses(String[] addresses) throws ArgumentException {
		...
		FindTransactionResponse ftr = findTransactions(addressesWithoutChecksum.toArray(new String[] {}), null, null,
				null);
	 ...
	 * Wrapper function for findTransactions, getTrytes and transactionObjects.
	 * Returns the transactionObject of a transaction hash. The input can be a list
	 * of valid bundles. findTransactions input
	 *
	 * @param bundles
	 *            The bundles.
	 * @return Transactions.
	 ...
	 *
	 * @param seed
	 *            Tryte-encoded private key / seed.
	 * @param security
	 *            The security level of private key / seed.
	 * @param transfers
	 ...
	 *            whether or not to validate the balances of the provided inputs
	 * @return Returns bundle trytes.
	 * @throws ArgumentException
	 *             is thrown when the specified input is not valid.
	 */
	public List<String> prepareTransfers(String seed, int security, final List<Transfer> transfers, String remainder,
			List<Input> inputs, boolean validateInputs) throws ArgumentException {
		...
		for (final Transfer transfer : transfers) {
			...
			// If message longer than 2187 trytes, increase signatureMessageLength (add 2nd
			// transaction)
	 ...
	 * @throws ArgumentException
	 *             is thrown when the specified security level is not valid.
	 **/
	public GetBalancesAndFormatResponse getBalanceAndFormat(final List<String> addresses, long threshold, int start,
			StopWatch stopWatch, int security) throws ArgumentException, IllegalStateException {
	 ...
	 * @return an array of bundle, if there are multiple arrays it means that there
	 *         are conflicting bundles.
	 * @throws ArgumentException
	 *             is thrown when the specified input is not valid.
	 */
	public GetBundleResponse getBundle(String transaction) throws ArgumentException {
		...
		if (!(bundle.getTransactions().get(bundle.getLength() - 1)
				.getCurrentIndex() == (bundle.getTransactions().get(bundle.getLength() - 1).getLastIndex())))
			throw new ArgumentException(INVALID_BUNDLE_ERROR);

		// Validate the signatures
		for (Signature aSignaturesToValidate : signaturesToValidate) {
			String[] signatureFragments = aSignaturesToValidate.getSignatureFragments()
					.toArray(new String[aSignaturesToValidate.getSignatureFragments().size()]);
			String address = aSignaturesToValidate.getAddress();
			boolean isValidSignature = new Signing(customCurl.clone()).validateSignatures(address, signatureFragments,
					bundleHash);
	...
	public GetAccountDataResponse getAccountData(String seed, int security, int index, boolean checksum, int total,
		...
		return GetAccountDataResponse.create(gna.getAddresses(), gtr.getTransfers(), gbr.getInputs(),
				gbr.getTotalBalance(), stopWatch.getElapsedTimeMili());
	}
	 ...
	 *             is thrown when the specified input is not valid.
	 */
	public ReplayBundleResponse replayBundle(String transaction, int depth, int minWeightMagnitude)
			throws ArgumentException {
		...
		Collections.reverse(bundleTrytes);
		List<Transaction> trxs = sendTrytes(bundleTrytes.toArray(new String[bundleTrytes.size()]), depth,
				minWeightMagnitude);
	 ...
	 *            If defined, this remainderAddress will be used for sending the
	 *            remainder value (of the inputs) to.
	 * @param validateInputs
	 *            Whether or not to validate the balances of the provided inputs.
	 * @param validateInputAddresses
	 *            Whether or not to validate if the destination address is already
	 *            used, if a key reuse is detect ot it's send to inputs.
	...
	public SendTransferResponse sendTransfer(String seed, int security, int depth, int minWeightMagnitude,
			final List<Transfer> transfers, List<Input> inputs, String remainderAddress, boolean validateInputs,
			boolean validateInputAddresses) throws ArgumentException {
	 ...
	 *
	 * @param trunkTx
	 *            Hash of a trunk or a tail transaction of a bundle.
	 * @param bundleHash
	 *            The bundle hashes.
	 * @param bundle
	 *            List of bundles to be populated.
	 * @return Transaction objects.
	 * @throws ArgumentException
	 *             is thrown when an invalid input is provided.
	 */
	 ...
	 *
	 * @param securitySum
	 *            The sum of security levels used by all co-signers.
	 * @param inputAddress
	 *            Array of input addresses as well as the securitySum.
	 * @param remainderAddress
	 *            Has to be generated by the cosigners before initiating the
	 *            transfer, can be null if fully spent.
	 * @return Bundle of transaction objects.
	 * @throws ArgumentException
	 *             is thrown when an invalid argument is provided.
	 * @throws IllegalStateException
	 *             is thrown when a transfer fails because their is not enough
	...
	public List<Transaction> initiateTransfer(int securitySum, final String inputAddress, String remainderAddress,
		...
		for (final Transfer transfer : transfers) {
			...
			// If message longer than 2187 trytes, increase signatureMessageLength (add next
			// transaction)
			if (transfer.getMessage().length() > Constants.MESSAGE_LENGTH) {
	 ...
	 *             balance to perform the transfer.
	 */
	public List<String> addRemainder(final String seed, final int security, final List<Input> inputs,
			final Bundle bundle, final String tag, final long totalValue, final String remainderAddress,
